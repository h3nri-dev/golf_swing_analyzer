...
| Current-frame and per-keyframe feedback (`Rt`, `ne`) | No posture observations or phase guidance | Current-frame observations, measured differences from address, guidance for each phase, optional notes on thumbnails, notes in enlarged views and PDF. Missing tracking stays explicitly unavailable. |
...
| Range looping (original time-update handler checks `trimStart`/`trimEnd`) | Selected range affected analysis only | Explicit **Loop window** beside the range; pins boundaries. Independent loops leave the other player/common display alone. Linked loops use the overlapping calibrated windows. Follow ±5s exits looping. |
...
- **Single/compare, independent players and common controls:** retained, along with FPS-aware timing, slow-motion calibration, persistent zoom/pan, mirror, large-screen layout, draggable ±5-second analysis windows and exact boundaries.
...
- **Trim timeline magnification:** replaced by the broad sliding analysis window, exact time inputs, single-frame stepping and moment jumps. Spatial video zoom is separate. The timeline remains a full-clip overview; it does not pretend to zoom time. Looping is explicit instead of starting implicitly after Analyze.
...
- The user's proximity rule governs placement: marking/jumping/syncing on moment cards; drawing tools at the video; transforms at the selected drawing; looping at the range; range directly above playback; global synchronization beside global playback.
