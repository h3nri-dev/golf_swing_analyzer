...
- Every keyframe card shows its phase-specific results automatically, with separate A/B values in comparison. **Results** on that card opens all eight measurements, changes from address and review guidance alongside the enlarged frame. Overlay affects only preview overlays; results require no toggle. Editing a marker or changing handedness refreshes that frame's results. On short laptops, comparison cards show frame numbers (hover for the full timestamp), and compact Zoom − / value / + / Fit controls preserve video height. PDF reports print all available moments with the same complete frame analysis.
