...
Open **Draw** for color, stroke, visibility duration, copy, hide, delete and image export. The A/B badge on each card chooses the editing target. Comparison always shows individual players beneath the videos and a common controller below the pair. **Back to video** or Escape gives videos the sidebar's space without stopping playback, resetting zoom or moving drawings. On phones and tablets, controls open over the footage; close them or choose a drawing tool to return to the unobstructed video. All five panel tabs remain accessible in the header.
...
The studio fills one browser viewport. Opening a video brings it into view; native scroll snapping lands on the studio when scrolling from the introduction or guide. The page can still scroll to those sections. Common playback and Analyze/Cancel remain at the bottom; each comparison video also keeps its own player controls. Draw, Video, Range, Results and Moments tabs expose the selected controls within that screen. Tabs support arrow keys; Escape closes the panel. At unusually small sizes or high browser text zoom, a panel can scroll internally so controls remain accessible.
