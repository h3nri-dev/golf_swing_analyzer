...
- **Single video:** choose a local clip, pause at your swing, then choose **Analyze**. By default it scans 0.3 real seconds before and 3.2 after that frame, clipped at the video’s edges. Use **Results** to select your playing hand and inspect all eight image angles, phase observations and head/both-hand paths.
- **Compare videos:** load A and B. **Sync on** aligns both clips at their first shared marker—Address first, then the next numbered phase present in both—and enables shared playback/stepping. Automatic estimates and your edits count; unconfirmed range previews do not. Both videos pause at those markers, using each clip's File FPS / Shot FPS calibration. Clicking Sync on again reapplies the current markers. With no shared marker, the existing sync points are retained. To align another pair of frames, choose **Sync off**, position each clip at the same event (usually impact), then choose **Sync Videos**. This always aligns their currently displayed frames, including when sync is already on. Press Play both to resume. Synchronized playback stays within the overlapping range and stops when either clip reaches the end.
...
- **Sync on/off:** turning sync off preserves current playback and zoom. Turning it on pauses both at their first shared marker and uses the selected clip’s speed. Without a shared marker, it retains the existing offset and aligns at the selected clip’s position within the shared range. Press Play both to resume. With sync off, marking, replacing or reaching the end of one clip leaves the other playing.
...
The playback scrubber also selects the analysis interval. Its translucent light-green band covers **0.3 real seconds before and 3.2 after the current frame**, with a tall vertical line at the playhead. Drag anywhere on the 48-pixel-high bar to seek and move the window together; it continues following during playback. Clip edges shorten the window. **Analyze** sits immediately after **Speed**, with a matching action in each comparison player's controls. The common controller has an adjacent A/B analysis target.

Adjust the **− / + seconds** inputs beside the timeline to change those offsets. They apply to both videos and save in this browser for later visits. Non-negative offsets must total 0.1–20 seconds. If storage is unavailable, adjustments still work for the current page. These are durations around the playhead, not separate absolute start/end points. There is no extra range slider or pin/follow toggle. Arrow keys on a focused scrubber move one file frame; Shift + arrow and Page Up/Down move one real second. Home/End seek to the displayed timeline's edges. File FPS / Shot FPS immediately recalibrate the window in both modes, including the independent common controller. A 120 FPS recording saved at 30 FPS has a default window spanning 14 file seconds (3.5 real seconds); changing File FPS to 60 reduces that to seven file seconds. Ordinary 30/60 FPS clips both cover the same 3.5 real seconds. Both clips maintain their own following windows. With Sync off, local playback leaves the common clock, window and playback state unchanged. Explicit FPS edits update the common window's scale only when that clip supplies its clock, preserving its saved file position and playback state.
...
After analyzing part of a clip, its playback timeline automatically expands to that **Analyzed range**. Start/end times and magnification appear beside the timeline. Numbered, color-coded markers match the seven moment cards; nearby labels spread apart with lines pointing to their exact timestamps. Click a marker to jump to its frame. **Analyzed range** plays and loops only inside the saved interval. **Full video** restores unrestricted playback and the overview. Paused playheads and spatial video zoom stay put when switching views; starting playback from outside a selected range returns to its start. The saved review interval stays fixed even as the next analysis window follows the playhead. Sync off lets each video loop independently; Sync on loops the overlapping aligned intervals using each video’s FPS calibration. If those intervals do not overlap, align the swings or choose Full video before playing. Cancellation retains the previous review window.
...
- **Loop window**, beside playback, captures the current analysis interval and repeats it during playback. Independently loop either video, or use Sync on to loop their overlapping calibrated intervals. In Analyzed range, this control reads **Looping range** and switches to Full video when clicked. The green analysis window always follows playback separately.
