...
- **Single video:** choose a local clip, select a section in **Analyze range** directly above playback, then choose **Analyze**. Only that section is scanned. Use **Results** to select your playing hand and inspect pose landmarks, the lead-hand trail and image-plane joint angles.
...
The expanded **Analyze range** row sits immediately above the common player bar. Its A/B selector, boundary controls and Analyze/Cancel buttons stay together. Draw, Video, Results and Moments remain expanded in the right sidebar. Drag the two handles to highlight any section of up to **20 real seconds**, or enter exact start/end times in real seconds. Dragging a handle previews that boundary; keyboard arrow keys adjust a focused handle. You can also pause or step to a moment and choose **Set** for Start or End. **Go** beside Start or End revisits either boundary without changing your selection. The playhead remains visible on the range bar.
