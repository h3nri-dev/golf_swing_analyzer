...
export function createStudioScreen({ slots, state, changed }) {
  ...
  let current = 'range', collapsed = compact.matches, scheduled = false;
  const definitions = [['draw','Drawing'],['video','Video'],['range','Range'],['pose','Pose'],['moments','Moments']];
  ...
  commands.append($('analyze'), $('cancel')); footer.querySelector('.transport-row').append(commands);
  ...
  $('analyze').textContent = 'Analyze range';
  ...
  panels.pose.append(pieces.hand, pieces.overlays, pieces.metrics, pieces.note);
  panels.moments.append(pieces.momentsHeading, pieces.momentsCopy, pieces.phases, pieces.tempo, pieces.tempoNote, pieces.export);
  const videoNote=document.createElement('p');videoNote.className='screen-panel-help';videoNote.textContent='Settings for the selected swing. Mark the same event in both clips to align them.';panels.video.append(videoNote);
  slots.forEach((s,i)=>{
    ...
    const speed=s.get('.clip-speed').closest('label');
    // Warm the selectors before moving their nodes out of the video card.
    for(const selector of ['.sync-mark','.mirror','.fps','.fps-label','.clip-speed','.zoom-pan','.zoom-fit']) s.get(selector);
    group.append(s.get('.sync-mark'), s.get('.mirror'), s.get('.fps-label'), speed, s.get('.zoom-pan'), s.get('.zoom-fit'));
  ...
  inspectorHeader.innerHTML='<h2 id="screenPanelTitle">Analysis range</h2><button id="closePanel" aria-label="Close controls panel">×</button>';
  ...
  function close() { collapsed=true;apply(); }
  tabs.addEventListener('click',e=>{const b=e.target.closest('[data-panel]');if(b) {if(!collapsed&&b.dataset.panel===current)close();else open(b.dataset.panel);}});
  ...
  $('toggleInsights').onclick=()=>{collapsed=!collapsed;apply();};$('closePanel').onclick=close;
  studio.addEventListener('click',e=>{if(e.target.closest('[data-tool]')) {if(compact.matches)close();else open('draw');}});
  ...
  compact.addEventListener('change',()=>{collapsed=compact.matches;apply();});
