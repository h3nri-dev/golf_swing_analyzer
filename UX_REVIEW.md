...
Phase estimates and user-confirmed marks have distinct labels. If the hand path lacks a sufficiently clear swing progression, the six sampled frames are called Range previews, with a short explanation and editing access. Manual marks remain authoritative until a new analysis completes; the subsequent user-requested behavior replaces them with the new results on completion. The restored previews, including range fallbacks and optional downswing/follow-through marks, are included in PDF reports.
