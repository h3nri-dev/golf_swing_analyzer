...
| --- | --- | --- |
| The linked comparison displayed a shared Play control and a Play both button under each video. | Three buttons appear to have different purposes although they do the same thing. | One shared transport in single/linked mode. Sync off shows each clip's transport and speed beside that clip; the shared transport is hidden. |
...
- **Show state and provide recovery.** [NN/g, 10 Usability Heuristics](https://www.nngroup.com/articles/ten-usability-heuristics/) informed the selected-swing labels, alignment feedback, meaningful empty states, consistent exits and protection against losing session work.
