...
- `deploy/moments.js`: six visible, color-coded moment cards with adjacent Set A/Set B buttons; no phase dropdowns or distant marking controls; frame editing and jumps must remain local when Sync is off. Display clocks, markers and ranges in real seconds and label zero-based frame numbers.
