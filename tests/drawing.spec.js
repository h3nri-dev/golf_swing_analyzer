...
test('frame annotations return at their marked time and survive responsive resizing and mirror',async({page})=>{
  ...
  const original=await exported(page);await (await settings(page,0,'.mirror')).click();await focusSection(page,'draw');await page.locator('#drawingScope').selectOption('clip');await page.locator('[data-tool="line"]').click();await line(page,0,{x:.1,y:.2},{x:.4,y:.5});
