...
test('Analyze captures the live ±5s window, freezes scanning, and preserves previous results on cancellation',async({page})=>{
 await stub(page);await page.goto('/');await load(page);await bounds(page,0,5);
 ...
 await expect(page.locator('#timeline')).toBeDisabled();await bounds(page,27,37);
 ...
 const scanned=await page.evaluate(()=>window.scans);expect(scanned).toHaveLength(240);expect(Math.min(...scanned)).toBe(27);expect(Math.max(...scanned)).toBeLessThan(37);
 ...
 await expect.poll(async()=>(await data(page)).selectedRange[0]).toBeGreaterThan(54.05);
 ...
 expect((await data(page)).analyzedRange).toEqual([27,37]);
 const at=await card(page).locator('video').evaluate(v=>v.currentTime);expect(frozen[0]).toBeCloseTo(at-5,5);expect(frozen[1]).toBe(60);
});
...
test('a single tall scrubber moves both the playhead and window, including keyboard frames, without losing zoom',async({page})=>{
 ...
 const at=await card(page).locator('video').evaluate(v=>v.currentTime);expect(at).toBeCloseTo(30,0);await bounds(page,at-5,at+5);
 ...
 await page.locator('#play').click();await expect.poll(async()=>(await data(page)).selectedRange[0]).toBeGreaterThan(at-5+1.1);await page.locator('#play').click();
...
test('analysis from playback restores the exact anchor without a false stale-results warning',async({page})=>{
 ...
 await expect.poll(async()=>(await data(page)).selectedRange[0]).toBeGreaterThan(20.1);
 await page.locator('#analyze').click();await expect(page.locator('#status')).toContainText('No clear pose',{timeout:15000});
 const report=await data(page),at=await card(page).locator('video').evaluate(v=>v.currentTime);expect(report.analyzedRange[0]).toBeCloseTo(at-5,5);expect(report.analyzedRange[1]).toBeCloseTo(at+5,5);await expect(page.locator('#analyzedRangeNote')).toBeHidden();
});
