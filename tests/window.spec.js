...
test('Analyze captures the live −0.3s / +3.2s window, freezes scanning, and preserves previous results on cancellation',async({page})=>{
 await stub(page);await page.goto('/');await load(page);await bounds(page,0,3.2);
 await page.locator('#timeline').fill('30');await bounds(page,29.7,33.2);
 await page.evaluate(()=>{document.querySelector('video').currentTime=32;document.getElementById('analyze').click();});
 await expect(page.locator('#timeline')).toBeDisabled();await bounds(page,31.7,35.2);
 await expect(page.locator('#status')).toContainText('No clear pose',{timeout:15000});
 expect((await data(page)).analyzedRange).toEqual([31.7,35.2]);
 const scanned=await page.evaluate(()=>window.scans);expect(scanned).toHaveLength(105);expect(Math.min(...scanned)).toBe(31.7);expect(Math.max(...scanned)).toBeLessThan(35.2);
 expect(await card(page).locator('video').evaluate(v=>v.currentTime)).toBe(32);
 await page.locator('#commonPlayer .timeline-full').click();await page.locator('#timeline').fill('59');await bounds(page,58.7,60);
 ...
 await expect.poll(async()=>(await data(page)).selectedRange[0]).toBeGreaterThan(58.75);
 ...
 expect((await data(page)).analyzedRange).toEqual([31.7,35.2]);
 const at=await card(page).locator('video').evaluate(v=>v.currentTime);expect(frozen[0]).toBeCloseTo(at-.3,5);expect(frozen[1]).toBe(60);
});
...
test('a single tall scrubber moves both the playhead and window, including keyboard frames, without losing zoom',async({page})=>{
 ...
 const at=await card(page).locator('video').evaluate(v=>v.currentTime);expect(at).toBeCloseTo(30,0);await bounds(page,at-.3,at+3.2);
 ...
 await page.locator('#play').click();await expect.poll(async()=>(await data(page)).selectedRange[0]).toBeGreaterThan(at-.3+1.1);await page.locator('#play').click();
...
test('each unsynced player owns its FPS-aware window and local playback leaves the common window unchanged',async({page})=>{
 ...
 await card(page,0).locator('.clip-timeline').fill('20');await bounds(page,19.7,23.2);
 await card(page,1).locator('.clip-timeline').fill('40');await bounds(page,39.7,43.2,1);
 await card(page,1).locator('.shot-fps').selectOption('120');await bounds(page,38.8,52.8,1);
 // Explicit calibration updates the common clock's scale at its saved frame.
 await expect(commonRail).toHaveAttribute('data-time','0');await expect(commonRail).toHaveAttribute('data-end','12.8');
 const common=await commonRail.evaluate(e=>({...e.dataset}));
 const band=card(page,1).locator('.timeline-window');expect(parseFloat(await band.evaluate(e=>e.style.width))).toBeCloseTo(14/60*100,3);
 await card(page,1).locator('.clip-speed').selectOption('0.25');await card(page,1).locator('.clip-play').click();
 await expect.poll(async()=>(await data(page,1)).selectedRange[0]).toBeGreaterThan(38.9);
 ...
 await expect(page.locator('#status')).toContainText('No clear pose',{timeout:15000});expect((await data(page)).analyzedRange).toEqual([19.7,23.2]);expect((await data(page,1)).analyzedRange).toBeNull();
...
test('analysis from playback restores the exact anchor without a false stale-results warning',async({page})=>{
 ...
 await expect.poll(async()=>(await data(page)).selectedRange[0]).toBeGreaterThan(24.8);
 await page.locator('#analyze').click();await expect(page.locator('#status')).toContainText('No clear pose',{timeout:15000});
 const report=await data(page),at=await card(page).locator('video').evaluate(v=>v.currentTime);expect(report.analyzedRange[0]).toBeCloseTo(at-.3,5);expect(report.analyzedRange[1]).toBeCloseTo(at+3.2,5);await expect(page.locator('#analyzedRangeNote')).toBeHidden();
});
