...
async function load(page,i) { await clip(page,i).locator('input[type=file]').setInputFiles(fixture); await discardIfAsked(page);await expect(clip(page,i).locator('video')).toBeVisible(); await expect(page.locator('#analyze')).toBeEnabled(); }
async function seek(page,t) { await (await transport(page,'timeline')).fill(String(t)); await page.waitForTimeout(120); }
...
test('single mode loads local video, steps, marks tempo and exports', async ({ page }) => {
  ...
  await (await settings(page,0,'.fps')).selectOption('60'); await seek(page,0); await (await transport(page,'next')).click(); expect((await times(page))[0]).toBeCloseTo(1/30,3);
...
test('comparison aligns offsets, steps together, plays independently and pauses on mode change', async ({page})=>{
  ...
  await page.locator('#independent').click(); await page.locator('[data-select="0"]').click(); await seek(page,.5);
  await page.locator('[data-select="1"]').click(); await seek(page,1.2);  await page.locator('#align').click();
  let ts = await times(page); expect(ts[1]-ts[0]).toBeCloseTo(.7,2);
  await (await transport(page,'next')).click(); ts=await times(page); expect(ts[1]-ts[0]).toBeCloseTo(.7,2);
  await (await transport(page,'play')).click(); await page.waitForTimeout(400); ts=await times(page); expect(Math.abs(ts[1]-ts[0]-.7)).toBeLessThan(.09);
  await page.locator('#independent').click(); expect(await page.locator('video').evaluateAll(v=>v.every(x=>!x.paused))).toBe(true);
  await (await transport(page,'play',1)).click(); const before=await times(page); await page.waitForTimeout(250); ts=await times(page); expect(ts[0]).toBeGreaterThan(before[0]); expect(ts[1]).toBeCloseTo(before[1],2);
...
test('failed model download preserves usable playback',async({page})=>{
  ...
  await page.goto('/'); await load(page,0); await page.locator('#analyze').click(); await expect(page.locator('#status')).toContainText('Analysis unavailable'); await expect((await transport(page,'play'))).toBeEnabled();
});
test('cancelling a stalled model download immediately restores the workspace',async({page})=>{
  ...
  await page.goto('/'); await load(page,0); await page.locator('#analyze').click(); await page.locator('#cancel').click(); await expect(page.locator('#status')).toContainText('Analysis cancelled'); await expect((await transport(page,'play'))).toBeEnabled();
});
test('real MediaPipe model detects a pose locally',async({page})=>{
  ...
  await focusSection(page,'range');await page.locator('#timeline').fill('0.2'); await page.locator('#analyze').click();
...
test('negative sync offsets respect overlap with unequal clip lengths',async({page})=>{
  ...
  await page.locator('#independent').click(); await page.locator('[data-select="0"]').click(); await seek(page,1);
  await page.locator('[data-select="1"]').click(); await seek(page,.2);  await page.locator('#align').click();
  ...
  await seek(page,2.7); await (await transport(page,'play')).click(); await page.waitForTimeout(500);
...
test('independent videos can both play and replacement clears only its own marks',async({page})=>{
  ...
  await focusSection(page,'range');await expect(page.locator('#rangeStart,#rangeEnd')).toHaveCount(0); await expect(page.locator('#analyze')).toBeEnabled(); await expect((await transport(page,'play'))).toBeEnabled();
});
...
test('real Detailed model produces visible image measurements locally',async({page})=>{
 ...
 await page.locator('#timeline').fill('0.2');await page.locator('.analysis-quality').first().selectOption('detailed');await page.locator('#analyze').click();
