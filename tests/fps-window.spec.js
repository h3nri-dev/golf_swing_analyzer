...
test('single-mode window and scan selection resize immediately for recording and file FPS',async({page})=>{
 ...
 await clip(page).locator('.fps').selectOption('60');await windowAt(rail,25,35);expect((await data(page)).selectedRange).toEqual([25,35]);
...
for(const clock of [0,1])test(`Sync off refreshes swing ${clock?'B':'A'} calibration without adopting its independently moved playhead`,async({page})=>{
 ...
 await windowAt(common,20,40);await windowAt(local,25,45);expect((await data(page,clock)).selectedRange).toEqual([25,45]);
 ...
 await clip(page,clock).locator('.fps').selectOption('60');await windowAt(common,25,35);await windowAt(local,30,40);expect((await data(page,clock)).selectedRange).toEqual([30,40]);
 ...
 await clip(page,clock).locator('.shot-fps').selectOption('same');await windowAt(common,27.5,32.5);await windowAt(local,32.5,37.5);
});
...
test('linked comparison resizes both windows in each clip calibration and changes back without drift',async({page})=>{
 ...
 await windowAt(clip(page,0).locator('.timeline-rail'),25,35);await windowAt(clip(page,1).locator('.timeline-rail'),25,35);await windowAt(page.locator('#commonPlayer .timeline-rail'),25,35);
