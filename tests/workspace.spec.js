...
test('hiding insights expands videos without interrupting playback or changing zoom', async ({page}) => {
  ...
  await page.locator('#speed').selectOption('0.25'); await clip(page,0).locator('.zoom-slider').fill('2');
  const before=await clip(page,0).locator('.stage').boundingBox();
  await clip(page,0).locator('.clip-play').click(); await page.locator('#toggleInsights').click();
  ...
  await page.locator('#toggleInsights').click(); await expect(page.locator('#analysisPanel')).toBeVisible();
