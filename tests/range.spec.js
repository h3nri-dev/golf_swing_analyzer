...
test('choose boundaries by handles and current frame without losing zoom', async ({page}) => {
  ...
  await page.locator('#rangeStartHandle').fill('0.5'); await page.locator('#rangeEndHandle').fill('1.5');
  ...
  await page.locator('#timeline').fill('0.75'); await page.locator('#rangeSetStart').click();
  await page.locator('#timeline').fill('2'); await page.locator('#rangeSetEnd').click();
  ...
  await page.locator('#rangeSelection').screenshot({path:'/tmp/swing-range-desktop.png'});
...
test('invalid and empty ranges cannot start analysis', async ({page}) => {
  ...
  await page.locator('#rangeEnd').fill('1'); await page.locator('#rangeStart').fill('2');
  await expect(page.locator('#rangeError')).toHaveText('End must be after start.');
  await expect(page.locator('#analyze')).toBeDisabled(); await expect(page.locator('#analyzeSelection')).toBeDisabled();
  await page.locator('#rangeEnd').fill('5'); await expect(page.locator('#rangeError')).toContainText('within the video');
  await page.locator('#rangeStart').fill(''); await expect(page.locator('#rangeError')).toHaveText('Enter a start and end time.');
  await page.locator('#rangeReset').click(); await expect(page.locator('#analyzeSelection')).toBeEnabled(); await expect(page.locator('#rangeError')).toBeHidden();
});

test('comparison ranges remain separate and unsynced boundary previews leave the other clip playing', async ({page}) => {
  await page.goto('/'); await load(page, 0); await page.locator('#rangeEnd').fill('1');
  ...
  await clip(page, 1).locator('.clip-speed').selectOption('0.25'); await clip(page, 1).locator('.clip-play').click();
  await page.locator('[data-select="0"]').click(); await expect(page.locator('#rangeEnd')).toHaveValue('1.000');
  await page.locator('#rangeStartHandle').fill('0.5'); expect(await clip(page, 1).locator('video').evaluate(v => v.paused)).toBe(false);
...
test('analysis scans only the selected interval and exports the result range after edits or cancellation', async ({page}) => {
  ...
  await page.locator('#timeline').fill('2.5'); await page.locator('#rangeStart').fill('1'); await page.locator('#rangeEnd').fill('1.3');
  await page.locator('#analyzeSelection').click(); await expect(page.locator('#status')).toContainText('No clear pose found');
  ...
  await page.locator('#rangeEnd').fill('3'); await expect(page.locator('#analyzedRangeNote')).toContainText('Analyze again');
  data = await exported(page); expect(data.range).toEqual([1,1.3]); expect(data.selectedRange).toEqual([1,3]);
  await page.locator('#analyzeSelection').click(); await expect(page.locator('#rangeStart')).toBeDisabled();
  await expect(page.locator('#rangeStatus')).toBeVisible();
  await page.locator('#cancelSelection').click(); await expect(page.locator('#rangeStatus')).toContainText('Analysis cancelled');
...
test('range handles support touch and keyboard on a phone without overflow', async ({browser}) => {
  ...
  await page.locator('#rangeSelection').scrollIntoViewIfNeeded();
  ...
  await page.locator('#rangeSelection').screenshot({path:'/tmp/swing-range-phone.png'}); await context.close();
});
