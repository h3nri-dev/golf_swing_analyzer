...
test('short videos safely use the whole clip and removal disables analysis',async({page})=>{
 ...
 for(const time of ['0','2','4']){await page.locator('#timeline').fill(time);expect((await report(page)).selectedRange).toEqual([0,4]);await expect(page.locator('#analyze')).toBeEnabled();}
...
test('merged playback and analysis scrubber accepts touch dragging on a phone',async({browser})=>{
 ...
 const at=await clip(page).locator('video').evaluate(v=>v.currentTime),range=(await report(page)).selectedRange;expect(at).toBeCloseTo(30,0);expect(range[0]).toBeCloseTo(at-5,3);expect(range[1]).toBeCloseTo(at+5,3);
