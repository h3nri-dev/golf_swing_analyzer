...
test('short-video windows clip safely at both edges and removal disables analysis',async({page})=>{
 ...
 for(const [time,expected] of [['0',[0,3.2]],['2',[1.7,4]],['4',[3.7,4]]]){await page.locator('#timeline').fill(time);expect((await report(page)).selectedRange).toEqual(expected);await expect(page.locator('#analyze')).toBeEnabled();}
...
test('merged playback and analysis scrubber accepts touch dragging on a phone',async({browser})=>{
 ...
 const at=await clip(page).locator('video').evaluate(v=>v.currentTime),range=(await report(page)).selectedRange;expect(at).toBeCloseTo(30,0);expect(range[0]).toBeCloseTo(at-.3,3);expect(range[1]).toBeCloseTo(at+3.2,3);
