...
test('choose boundaries by handles and current frame without losing zoom', async ({page}) => {
  ...
  await openPanel(page,'range');await page.locator('#rangeStartHandle').fill('0.5'); await page.locator('#rangeEndHandle').fill('1.5');
...
test('invalid and empty ranges cannot start analysis', async ({page}) => {
  ...
  await openPanel(page,'range');await page.locator('#rangeEnd').fill('1'); await openPanel(page,'range');await page.locator('#rangeStart').fill('2');
  ...
  await openPanel(page,'range');await page.locator('#rangeEnd').fill('5'); await expect(page.locator('#rangeError')).toContainText('within the video');
  await openPanel(page,'range');await page.locator('#rangeStart').fill(''); await expect(page.locator('#rangeError')).toHaveText('Enter a start and end time.');
...
test('analysis scans only the selected interval and exports the result range after edits or cancellation', async ({page}) => {
  ...
  await openPanel(page,'range');await page.locator('#rangeEnd').fill('3'); await expect(page.locator('#analyzedRangeNote')).toContainText('Analyze again');
...
test('range handles support touch and keyboard on a phone without overflow', async ({browser}) => {
  ...
  await openPanel(page,'range');await page.locator('#rangeSelection').scrollIntoViewIfNeeded();
  ...
  await openPanel(page,'range');await page.locator('#rangeSelection').screenshot({path:'/tmp/swing-range-phone.png'}); await context.close();
});
