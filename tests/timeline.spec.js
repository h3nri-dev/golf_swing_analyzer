...
test('analysis expands the timeline and separated markers seek exact frames without moving the review window',async({page})=>{
 ...
 const common=page.locator('#commonPlayer'),track=page.locator('#timeline');await bounds(track,27.5,32.5);
 ...
 await page.locator('#play').click();await expect.poll(async()=>await clip(page).locator('video').evaluate(v=>v.currentTime)).toBeGreaterThan(report.phaseTimes.impact+.05);await page.locator('#play').click();await bounds(track,27.5,32.5);
...
test('cancellation preserves the review window and FPS calibration changes clocks without changing its saved frames',async({page})=>{
 ...
 await clip(page).locator('.shot-fps').selectOption('120');await bounds(page.locator('#timeline'),6.875,8.125);expect((await model(page)).analyzedRange).toEqual([27.5,32.5]);
