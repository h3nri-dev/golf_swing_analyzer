...
async function fixedHighlight(host,start,end){
 const rail=host.locator('.timeline-rail'),band=rail.locator('.timeline-window');
 await expect(rail).toHaveAttribute('data-start',String(start));await expect(rail).toHaveAttribute('data-end',String(end));
 const r=await rail.boundingBox(),b=await band.boundingBox();expect(b.x).toBeCloseTo(r.x,1);expect(b.width).toBeCloseTo(r.width,1);
}
...
test('analysis expands the timeline and separated markers seek exact frames without moving the review window',async({page})=>{
 ...
 const common=page.locator('#commonPlayer'),track=page.locator('#timeline');await bounds(track,27.5,32.5);await fixedHighlight(common,27.5,32.5);
 await expect(common.locator('.timeline-bounds')).toHaveText('27.500–32.500 s · 12×');
 ...
 expect(await clip(page).locator('video').evaluate(v=>v.currentTime)).toBeCloseTo(report.phaseTimes.impact,4);await bounds(track,27.5,32.5);
 await page.locator('#play').click();await expect.poll(async()=>await clip(page).locator('video').evaluate(v=>v.currentTime)).toBeGreaterThan(report.phaseTimes.impact+.05);await fixedHighlight(common,27.5,32.5);await page.locator('#play').click();await bounds(track,27.5,32.5);
 const before=await clip(page).locator('video').evaluate(v=>v.currentTime);await common.locator('.timeline-full').click();await bounds(track,0,60);expect(await clip(page).locator('video').evaluate(v=>v.currentTime)).toBe(before);expect((await common.locator('.timeline-window').boundingBox()).width).toBeLessThan((await common.locator('.timeline-rail').boundingBox()).width);
 await common.locator('.timeline-range').click();await bounds(track,27.5,32.5);await expect(clip(page).locator('.zoom-value')).toHaveText('2.00×');
...
test('local expanded timelines remain independent and common marker seeks honor slow-motion timing',async({page})=>{
 ...
 await bounds(clip(page,0).locator('.clip-timeline'),22.5,27.5);await bounds(clip(page,1).locator('.clip-timeline'),2.5,7.5);await bounds(page.locator('#timeline'),2.5,7.5);
 await fixedHighlight(clip(page,0),22.5,27.5);await fixedHighlight(clip(page,1),10,30);await fixedHighlight(page.locator('#commonPlayer'),10,30);
 ...
 for(const [width,height] of [[1440,900],[1280,720],[2560,1440],[390,844]]){
  ...
  if(width>900&&height<=900){
   ...
   await clip(page,0).locator('.timeline-full').click();await bounds(clip(page,0).locator('.clip-timeline'),0,60);await bounds(page.locator('#timeline'),2.5,7.5);
   await clip(page,0).locator('.timeline-range').click();await bounds(clip(page,0).locator('.clip-timeline'),22.5,27.5);expect(await clip(page,0).locator('video').evaluate(v=>v.currentTime)).toBe(current);await expect(page.locator('#linked')).toHaveAttribute('aria-pressed','true');
  }
 }
});
...
test('cancellation preserves the review window and FPS calibration changes clocks without changing its saved frames',async({page})=>{
 await setup(page);await page.locator('#timeline').fill('30');await analyze(page);await bounds(page.locator('#timeline'),27.5,32.5);
 await clip(page).locator('video').evaluate(v=>v.currentTime=42.5);await page.locator('#analyze').click();await page.locator('#cancel').click();await expect(page.locator('#status')).toContainText('cancelled');await bounds(page.locator('#timeline'),27.5,32.5);
 await clip(page).locator('.shot-fps').selectOption('120');await bounds(page.locator('#timeline'),6.875,8.125);expect((await model(page)).analyzedRange).toEqual([27.5,32.5]);await fixedHighlight(page.locator('#commonPlayer'),27.5,32.5);
 await page.locator('#commonPlayer .timeline-full').click();await bounds(page.locator('#timeline'),0,15);
 await page.locator('#timeline').fill('10.5');await analyze(page);await bounds(page.locator('#timeline'),8,13);
});
