...
- Keep phase-specific results visible on every keyframe card without a Notes toggle. The nearby Results action shows all eight measurements, measured changes from address and phase guidance; every frame's PDF page uses the same analysis. Never substitute the current playhead's results for a saved keyframe, use an unset address as frame zero, or grade a swing from arbitrary 2D angle thresholds.
